...
export const CONTRACT_ADDRESS =
  "CBZYEZGNAP6X7KG6ZS7MYUH33N2XW66Z35T46MNXKELROZMIJPSAJMBL";
...
// ============================================================
// Subscription DApp — Contract Methods
// ============================================================

/**
 * Create a new subscription.
 * Calls: create_subscription(subscriber: Address, amount: i128, interval: u64)
 */
...
/**
 * Pay for an existing subscription.
 * Calls: pay(subscriber: Address)
 */
export async function paySubscription(caller: string) {
  return callContract(
    "pay",
    [toScValAddress(caller)],
    caller,
 ...
 * Calls: cancel(subscriber: Address)
 */
export async function cancelSubscription(caller: string) {
  return callContract(
    "cancel",
    [toScValAddress(caller)],
    caller,
 ...
 * Calls: get_subscription(subscriber: Address) -> Subscription
 * Returns: { subscriber: string, amount: bigint, interval: number, last_payment: number } or null
 */
export async function getSubscription(caller?: string) {
  // Use caller address if provided, otherwise use a placeholder for read-only
  const addressToUse = caller || "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHQS2";
  return readContract(
    "get_subscription",
    [toScValAddress(addressToUse)],
    caller
  );
}
